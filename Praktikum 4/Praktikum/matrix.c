...
float AvgRow(Matrix M, IdxType i){
	...
	return ((float)sum/((float)COL_EFF(M)));
}
...
float AvgCol(Matrix M, IdxType j){
	...
	return ((float)sum/((float)ROW_EFF(M)));
...
void RotateMat(Matrix *m){
    ElType previous, current;
    IdxType rowstart = 0;
    IdxType colstart = 0;
    IdxType rowInnerCap = ROW_EFF(*m); // lebih satu dari index yang diproses
    IdxType colInnerCap = COL_EFF(*m); // lebih satu dari index yang diproses
    IdxType x;

    while (rowstart<rowInnerCap && colstart<colInnerCap){
        if (rowstart+1 == rowInnerCap || colstart+1 == colInnerCap){
            break;
        }
        else{
            previous = ELMT(*m,rowstart+1,colstart);

            for (x=colstart; x<colInnerCap; x++){
                current = ELMT(*m, rowstart, x);
                ELMT(*m, rowstart, x) = previous;
                previous = current;
            }
            rowstart++;

            for (x=rowstart;x<rowInnerCap;x++){
                current = ELMT(*m, x, colInnerCap-1);
                ELMT(*m, x, colInnerCap-1) = previous;
                previous = current;
            }
            colInnerCap--;

            if (rowstart < rowInnerCap){
                for (x=colInnerCap-1; x>=colstart; x--){
                    current = ELMT(*m, rowInnerCap-1, x);
                    ELMT(*m, rowInnerCap-1, x) = previous;
                    previous = current;
                }
            }
            rowInnerCap--;

            if (colstart<colInnerCap){
                for (x=rowInnerCap-1; x>=rowstart; x--){
                    current = ELMT(*m, x, colstart);
                    ELMT(*m, x, colstart) = previous;
                    previous = current;
                }
            }
            colstart++;
    ...
    // int x;
    // ElType previous, current;
    // while(depth<=size && size>1){
    //     previous = ELMT(*m, depth+1, depth);
    //     for (x=0;x<size;x++){
    //         current = ELMT(*m, depth, depth+x);
    //         ELMT(*m, depth, depth+x) = previous;
    //         previous = current;
    //     }
    //     for (x=1;x<size;x++){
    //         current = ELMT(*m, depth+x, (size-1)+depth);
    //         ELMT(*m, depth+x, (size-1)+depth) = previous;
    //         previous = current;
    //     }
    //     for (x=1;x<size;x++){
    //         current = ELMT(*m, (size-1)+depth, (size-1)+depth-x);
    //         ELMT(*m, (size-1)+depth,(size-1)+depth-x) = previous;
    //         previous = current;
    //     }
    //     for (x=1;x<size-1;x++){
    //         current = ELMT(*m, (size-1)+depth-x, depth);
    //         ELMT(*m, (size-1)+depth-x, depth) = previous;
    //         previous = current;
    //     }
